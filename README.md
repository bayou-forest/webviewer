...
### 概要
登録後1週間以内で、その端末でまだ詳細画面を開いていないコンテンツにはサムネイルに「NEW」バッジが表示されます。

### データベース拡張
SQLiteデータベース(`ratings.sqlite3`)に以下のカラムが追加されています：
- **created_at**: コンテンツがプログラムによって一覧に追加された日時（UNIXタイムスタンプ）

### 閲覧済み管理
閲覧済みかどうかは**ブラウザのlocalStorageで管理**されています。これにより：
- **端末ごとに独立**: 別の端末やブラウザからアクセスした場合は新着バッジが表示される
- **自動記録**: 詳細画面を開くと自動的にlocalStorageに記録される
- **リロード不要**: 詳細画面を新しいタブで開いても、一覧画面に戻ると即座にバッジが消える

### 動作
1. **登録日の記録**: ファイルが初めて走査されたときに`created_at`が記録されます
2. **バッジ表示条件**: 
   - 登録から7日（1週間）以内
   - かつ、その端末のブラウザで詳細画面をまだ開いていない（localStorage未登録）
3. **閲覧マーク**: 詳細画面を開くとlocalStorageに記録され、次回表示時からバッジが消えます
4. **既存データの扱い**: 既存のコンテンツには自動的に現在時刻で`created_at`が設定されます

### カードの色分け
- **再生回数0回**: 明るい青灰色（`#3a4a5f`）
- **再生回数1回**: 中間の青灰色（`#2a3545`）
- **再生回数2回以上**: 通常の色（`#1d212b`）

## コンテンツの並び替え機能

一覧画面で、コンテンツの表示順を以下の基準で並び替えることができます：
...
### 並び順
- **昇順**: 小さい値から大きい値へ（ファイル名はA→Z、日付は古い→新しい）
- **降順**: 大きい値から小さい値へ（ファイル名はZ→A、日付は新しい→古い）

並び替え設定は画面上部のドロップダウンメニューから選択でき、即座に反映されます。
